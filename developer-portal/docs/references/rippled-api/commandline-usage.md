...
| `--ledgerfile {FILE}` | Load the ledger version from the specified `{FILE}`, which must contain a complete ledger in JSON format. For an example of such a file, see the provided [`ledger-file.json`](https://github.com/XRPLF/xrpl-dev-portal/blob/master/content/_code-samples/rippled-cli/ledger-file.json). |
