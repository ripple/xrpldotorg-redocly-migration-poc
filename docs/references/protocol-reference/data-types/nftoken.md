...
<table>
  ...
  <tr>
   ...
   <td>If set, indicates that this NFToken can be transferred. This flag has no effect if the token is being transferred <em>from</em> the issuer or <em>to</em> the issuer.
   </td>
  </tr>
  <tr>
...
Replace the string `{nftokenid}` with the `NFTokenID` as a 64-byte hex string when you attempt to query information.
...
You create NFTs using the `NFTokenMint` transaction. You can optionally destroy `NFToken` objects using the `NFTokenBurn` transaction.
