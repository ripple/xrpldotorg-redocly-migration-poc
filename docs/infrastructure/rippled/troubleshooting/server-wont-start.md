...
- Specify the path to your preferred config file using the `--conf` [commandline option](../../../references/http-websocket-apis/commandline-usage.md).
...
RocksDB is no longer supported as a backend for [history sharding](../../ledger-history/history-sharding.md). If you have an existing configuration that defines a RocksDB shard store, the server fails to start.
