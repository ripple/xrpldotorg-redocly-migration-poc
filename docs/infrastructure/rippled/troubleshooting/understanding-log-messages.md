...
This is an important step in [Diagnosing Problems](../troubleshooting/diagnosing-problems.md) with `rippled`.
...
If your server always crashes on startup, see [Server Won't Start](../troubleshooting/server-wont-start.md) for possible cases.
...
This indicates that your server is having trouble requesting specific ledger data from its peers. If the ledger index is much lower than the most recent validated ledger's index as reported by the [server_info method][], this probably indicates that your server is downloading a [history shard](../../ledger-history/history-sharding.md).
...
Messages such as the following are caused by a bug in `rippled` v1.1.0 and earlier when downloading historical ledgers for [history sharding](../../ledger-history/history-sharding.md):
...
Messages such as the following occur when [online deletion is interrupted](../../ledger-history/online-deletion.md#interrupting-online-deletion):
...
Log messages such as the following are issued when the XRP Ledger detects potential transaction censorship. For more information about these log messages and the transaction censorship detector, see [Transaction Censorship Detection](../../transaction-censorship-detection.md).
...
| `lastRotated`    | [Ledger Index][] | The end of the ledger range in the ["old" (read-only) database](../../ledger-history/online-deletion.md#how-it-works). Online deletion deletes this ledger version and earlier. |
| `deleteInterval` | Number           | How many ledger versions to keep after online deletion. The [`online_delete` setting](../../ledger-history/online-deletion.md#configuration) controls this value. |
| `canDelete_`     | [Ledger Index][] | The newest ledger version that the server is allowed to delete, if using [advisory deletion](../../ledger-history/online-deletion.md#advisory-deletion). If not using advisory deletion, this value is ignored. |
...
To resolve this error, remove the `[amendments]` and `[veto_amendments]` stanzas from your config file. For more information, see [Amendment Voting](../../../amendments/amendments.md#amendment-voting).
...
- **XRPL Infrastructure:**
    - [Diagnosing Problems](../troubleshooting/diagnosing-problems.md)
    - [Capacity Planning](../installation/capacity-planning.md)
- **API Reference:**
    - [rippled API Reference](../../../references/http-websocket-apis/index.mdx)
