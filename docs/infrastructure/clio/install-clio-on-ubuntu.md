...
- You need gRPC access to one or more `rippled` servers in [P2P mode](../rippled/installation/index.mdx). The `rippled` servers can either be local or remote, but you must trust them. The most reliable way to do this is to [install `rippled` yourself](../rippled/installation/index.mdx).
...
- **Understanding the XRPL:**
    - [The Clio Server](../../concepts/understanding-xrpl/server/clio-server.md)
