...
Every transaction has the same set of common fields, plus additional fields based on the [transaction type](transaction-types.md). Field names are case-sensitive. The common fields for all transactions are:
...
| `Fee`                | String           | Amount            | _(Required; [auto-fillable][])_ Integer amount of XRP, in drops, to be destroyed as a cost for distributing this transaction to the network. Some transaction types have different minimum requirements. See [Transaction Cost](transaction-cost.md) for details. |
| `Sequence`           | Number           | UInt32            | _(Required; [auto-fillable][])_ The [sequence number](../../../references/protocol-reference/data-types/basic-data-types.md#account-sequence) of the account sending the transaction. A transaction is only valid if the `Sequence` number is exactly 1 greater than the previous transaction from the same account. The special case `0` means the transaction is using a [Ticket](tickets.md) instead _(Added by the TicketBatch amendment.)_. |
...
The `AccountTxnID` field lets you chain your transactions together, so that a current transaction is not valid unless the previous transaction sent from the same account has a specific transaction hash. <!--[transaction hash][identifying hash].-->
...
The `AccountTxnID` field cannot be used on transactions that use [Tickets](tickets.md). Transactions that use `AccountTxnID` cannot be placed in the [transaction queue](../server/transaction-queue.md).
...
* `Fee` - Automatically fill in the [Transaction Cost](transaction-cost.md) based on the network.
...
The [`Paths` field](../../../references/protocol-reference/transactions/transaction-types/payment.md#paths) of the [Payment transaction](../../../references/protocol-reference/transactions/transaction-types/payment.md) type can also be automatically filled in.
...
[AccountSet]: ../../../references/protocol-reference/transactions/transaction-types/accountset.md
[AccountSet transaction]: ../../../references/protocol-reference/transactions/transaction-types/accountset.md
[AccountSet transactions]: ../../../references/protocol-reference/transactions/transaction-types/accountset.md
