...
Set the `asfAuthorizedNFTokenMinter` flag (numeric value _10_).
...
This transaction blob is the same as the one used for the previous [`mintToken()`](mint-and-burn-nftokens.md#mint-token)function, with the addition of the `Issuer` field.
